...
print(f'length of train_sess： {len(tra_sess)}')
print(f'length of tes_sess： {len(tes_sess)}')
...
def obtian_tra():
    ...
    print(f'item_ctr: {item_ctr}')
    for i in range(50):
        print(f'train_seqs[i]:  {train_seqs[i]}')
...
tra = (tr_seqs, tr_labs)
tes = (te_seqs, te_labs)
print(f'len(tra_seqs): {len(tr_seqs)}')
print(f'len(te_seqs): {len(te_seqs)}')
...
if opt.dataset == 'diginetica':
    ...
    print(f'len(tr_seqs[-split4:]): {len(tr_seqs[-split4:])}')
    print(f'len(tr_seqs[-split64:]): {len(tr_seqs[-split64:])}')

    tra4, tra64 = (tr_seqs[-split4:], tr_labs[-split4:]), (tr_seqs[-split64:], tr_labs[-split64:])
    seq4, seq64 = tra_seqs[tr_ids[-split4]:], tra_seqs[tr_ids[-split64]:]
    print(f'tra4[1]: {tra4[1]}')
